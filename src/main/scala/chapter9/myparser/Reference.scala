...
object ReferenceTypes {
  ...
  /** ParseState wraps a location and provides some extra
    ...
    *  in Sliceable.scala add an isSliced Boolean flag to ParseState.
    */
  ...
  //If s2 is longer than s1, then it returns sd1.length
...
object Reference extends Parsers[Parser]  {
  ...
  def or[A](p: Parser[A], p2: => Parser[A]): Parser[A] = s => p(s) match {
    case Failure(e, false) => p2(s)
    case r => r//Committed failure or succedss skips running p2
  }

  def flatMap[A, B](f: Parser[A])(g: A => Parser[B]): Parser[B] = s => f(s) match {
